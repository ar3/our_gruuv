class RefreshNamesSyncProcessorJob < ApplicationJob
  queue_as :default
  ...
  def perform(bulk_sync_event_id, organization_id)
    ...
    # Process the sync
    processor = RefreshNamesSyncProcessor.new(bulk_sync_event, organization)
    ...
    Rails.logger.error "RefreshNamesSyncProcessorJob failed: #{e.message}"
