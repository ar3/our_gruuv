class Organizations::InsightsController < Organizations::OrganizationNamespaceBaseController
  ...
  def real_og_leaders
    ...
    @board = %w[ogos goals org_goals check_ins feedback_requests].include?(params[:board].to_s) ? params[:board].to_s : 'ogos'
    ...
    when 'feedback_requests'
      @real_og_leaders_feedback_requests = Insights::RealOgLeadersFeedbackRequestsBuilder.new(
        company: company,
        range: range
      ).call
    else
  ...
  def observations_rating_health_ratios(range)
    ...
    Insights::ObservationsRatingHealth.org_rating_health_ratio_rows(scope.group(:rating).count)
  end
