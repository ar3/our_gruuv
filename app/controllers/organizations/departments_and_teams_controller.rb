class Organizations::DepartmentsAndTeamsController < Organizations::OrganizationNamespaceBaseController
  ...
  def update
    ...
    # Ensure type is preserved if not in params (disabled fields don't submit)
    ...
    if @department_or_team.update(params_hash)
      redirect_to organization_departments_and_team_path(@organization, @department_or_team), notice: 'Department or team was successfully updated.'
    else
      set_available_parents
  ...
  def set_department_or_team
    # Try to find in descendants first (descendants returns an array, so filter for active)
    all_descendants = @organization.descendants
    @department_or_team = all_descendants.find { |org| org.id == params[:id].to_i && org.deleted_at.nil? }
    
    # If not found, check if it's the organization itself (shouldn't happen for depts/teams, but handle gracefully)
    @department_or_team ||= @organization if @organization.id == params[:id].to_i && @organization.deleted_at.nil?
