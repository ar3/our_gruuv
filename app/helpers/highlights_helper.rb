module HighlightsHelper
  def highlights_transaction_description(transaction)
    case transaction
    ...
    when RedemptionTransaction
      reward_name = transaction.highlights_redemption&.highlights_reward&.name || "reward"
  ...
  def highlights_format_delta(delta)
  ...
  def highlights_delta_class(delta)
    return "" if delta.nil? || delta == 0
    delta > 0 ? "text-success" : "text-danger"
  end

  def highlights_dollar_value(points)
    number_to_currency(points / 10.0)
  end

  def highlights_points_display(points)
    "#{points} points (#{highlights_dollar_value(points)})"
  end
