module NavigationHelper
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            label: 'My team goals',
            icon: 'bi-people',
            path: organization_goals_path(current_organization, owner_id: 'all_my_teams'),
            policy_check: -> { policy(current_company).view_goals? },
            active_check: -> { nav_my_team_goals_item_active? },
            coming_soon: false
          },
          {
            label: 'My department goals',
            icon: 'bi-building',
            path: organization_goals_path(current_organization, owner_id: 'my_department'),
            policy_check: -> { policy(current_company).view_goals? },
            active_check: -> { nav_my_department_goals_item_active? },
            coming_soon: false
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: 'bi-heart-pulse',
            path: organization_positions_health_path(current_organization),
            policy_check: -> { policy(current_organization).positions_health? },
            coming_soon: false
          },
          {
            label: 'Protect Flow',
  ...
  def nav_my_team_goals_item_active?
    return false unless current_organization
    goals_path_only = organization_goals_path(current_organization).split('?').first
    return false unless request.path == goals_path_only

    request.query_parameters['owner_id'] == 'all_my_teams'
  end

  def nav_my_department_goals_item_active?
    return false unless current_organization
    goals_path_only = organization_goals_path(current_organization).split('?').first
    return false unless request.path == goals_path_only
