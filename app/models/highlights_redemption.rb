# Represents a redemption of highlights points for a reward
# Tracks the status of fulfillment (pending -> processing -> fulfilled/failed)
class HighlightsRedemption < ApplicationRecord
  belongs_to :company_teammate
  belongs_to :organization
  belongs_to :highlights_reward
  has_one :redemption_transaction, class_name: 'RedemptionTransaction',
          foreign_key: :highlights_redemption_id, dependent: :restrict_with_error
  ...
  def reward_name
    highlights_reward&.name || "Unknown Reward"
  end
  ...
  def reward_belongs_to_organization
    return unless highlights_reward && organization

    unless highlights_reward.organization_id == organization_id
      errors.add(:highlights_reward, 'must belong to the same organization')
    end
  end
