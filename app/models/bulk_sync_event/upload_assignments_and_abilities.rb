class BulkSyncEvent::UploadAssignmentsAndAbilities < BulkSyncEvent
  def validate_file_type(file)
    file.content_type.in?(['text/csv', 'application/csv']) || file.original_filename.end_with?('.csv')
  end

  def process_file_for_preview
    parser = AssignmentsAndAbilitiesUploadParser.new(source_contents, organization)
  ...
  def process_upload_in_background
    AssignmentsAndAbilitiesUploadProcessorJob.perform_later(id, organization.id)
  end

  def display_name
    'Upload Assignments and Abilities'
  end
