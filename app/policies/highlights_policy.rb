...
class HighlightsPolicy < ApplicationPolicy
  # Manage rewards catalog and settings
  def manage_rewards?
    return false unless viewing_teammate.is_a?(CompanyTeammate)
    admin_bypass? || viewing_teammate.can_manage_highlights_rewards?
  end

  # Manage highlights settings for the organization
  def manage_settings?
    manage_rewards?
  end
  ...
  # Award bank points - requires highlights management permission
  ...
  class Scope < ApplicationPolicy::Scope
    def resolve
      ...
      if admin_bypass? || viewing_teammate.can_manage_highlights_rewards?
