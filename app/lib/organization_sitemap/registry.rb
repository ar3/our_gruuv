    ...
    def goals_section
      {
        ...
        pages: [
          ...
          page(
            ...
            label: "My team goals",
            icon: "bi-people",
            path: ->(ctx) { ctx.organization_goals_path(ctx.organization, owner_id: "all_my_teams") },
            policy: ->(ctx) { ctx.policy(ctx.company).view_goals? },
            goal: "See goals owned by every team you belong to.",
    ...
    def insights_section
      {
        ...
        pages: [
          ...
          insights_page(:expectations_compliance, "Expectations Compliance", "bi-clipboard-check", :organization_expectations_compliance_path,
            policy: ->(ctx) { ctx.policy(ctx.organization).expectations_compliance? },
            goal: "See who has signed their true job description and who is overdue for a signature.",
          ...
          insights_page(:teams_health, "Teams Health", "bi-heart-pulse", :organization_teams_health_path,
            ...
            goal: "See how many teams are healthy on Team-owned Goal Confidence.",
            synonyms: %w[teams health team health team goals health]),
          insights_page(:milestones_health, "Milestones Health", "bi-heart-pulse", :organization_milestones_health_path,
            policy: ->(ctx) { ctx.policy(ctx.organization).milestones_health? },
            goal: "Monitor required ability milestone coverage across employees.",
            synonyms: %w[milestones health milestone health ability milestones]),
          ...
          insights_page(:insights_values, "Values", "bi-heart", :organization_insights_values_path,
            ...
            synonyms: %w[values insights aspirational values values champions values awards values clarity]),
          ...
          insights_page(:insights_teams, "Teams", "bi-people", :organization_insights_teams_path,
            policy: ->(ctx) { ctx.policy(ctx.company).view_goals? },
          ...
          insights_page(:insights_acknowledgements, "Acknowledgements", "bi-hand-thumbs-up", :organization_insights_acknowledgements_path,
            policy: ->(ctx) { ctx.policy(ctx.company).show? },
            goal: "Review acknowledgement responses on finalized check-ins.",
            synonyms: %w[acknowledgements acknowledge check-in acknowledgement insights]),
    ...
    def admin_section
      {
        ...
        pages: [
          ...
          admin_page(:position_comparison, "Position Comparison", "bi-layout-split", :organization_position_comparison_path,
