...
RSpec.describe Organizations::KudosRewards::BankAwardsController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    it 'assigns total_points_to_give and total_points_to_redeem from ledgers' do
      create(:kudos_points_ledger, organization: organization, company_teammate: admin_teammate, points_to_give: 50, points_to_spend: 25)
      other = create(:company_teammate, person: create(:person), organization: organization)
      create(:kudos_points_ledger, organization: organization, company_teammate: other, points_to_give: 10, points_to_spend: 5)

      get :index, params: { organization_id: organization.id }

      expect(assigns(:total_points_to_give)).to eq(60)
      expect(assigns(:total_points_to_redeem)).to eq(30)
    end
  end
