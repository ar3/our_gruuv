  ...
  describe 'GET #execute_changes' do
    context 'when user has proper authorization' do
      before do
        ...
        # Force assignment_has_changes? to return true so the partial gets rendered
        allow_any_instance_of(Organizations::PeopleController).to receive(:assignment_has_changes?).and_return(true)
      end
      ...
      it 'successfully renders execute_changes template without NameError' do
        ...
        # Should render successfully now that maap_snapshot is passed to the partial
