...
RSpec.describe CheckIns::RequiredCheckInUrgencySort do
  describe ".sort_tuple" do
    it "orders obscured before blurred" do
    ...
    it "orders aspiration before assignment when clarity matches" do
      t = 5.days.ago
      a = described_class.sort_tuple(:obscured, :aspiration, t, nil)
      b = described_class.sort_tuple(:obscured, :assignment, t, nil)
      expect((a <=> b)).to eq(-1)
    end

    it "orders older finalized check-ins before newer when other keys match" do
      older = described_class.sort_tuple(:blurred, :assignment, 90.days.ago, nil)
      newer = described_class.sort_tuple(:blurred, :assignment, 5.days.ago, nil)
      expect((older <=> newer)).to eq(-1)
    end

    it "treats working_to_meet as higher urgency than other ratings when other keys match" do
      t = 10.days.ago
      wtm = described_class.sort_tuple(:blurred, :assignment, t, "working_to_meet")
      other = described_class.sort_tuple(:blurred, :assignment, t, "meeting")
