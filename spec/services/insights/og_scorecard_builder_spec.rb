...
RSpec.describe Insights::OgScorecardBuilder do
  ...
  describe '#call' do
    ...
    it 'includes activity rows, a Gruuv Health separator, and Required Clarity rows in the Check-ins section' do
      ...
      expect(data_rows[2][:key]).to eq('unique_teammates_position_check_in_finalized_90_days')
      expect(data_rows[3][:key]).to eq('unique_teammates_check_in_finalized_all_time')
      ...
      expect(check_ins_group[:rows][5]).to include(separator: true, label: 'Gruuv Health · Required Clarity Check-Ins')
    end
