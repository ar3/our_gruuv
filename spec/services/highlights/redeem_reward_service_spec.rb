...
RSpec.describe Highlights::RedeemRewardService do
  ...
  describe '.call' do
    context 'with valid redemption' do
      it 'returns a successful result' do
        ...
        expect(result.value[:redemption]).to be_a(HighlightsRedemption)
      ...
      it 'creates redemption with correct attributes' do
        ...
        expect(redemption.highlights_reward).to eq(reward)
      ...
      it 'creates transaction with correct attributes' do
        ...
        expect(transaction.highlights_redemption).to eq(result.value[:redemption])
      end
    ...
    context 'when teammate has insufficient points' do
      ...
      it 'does not create any records' do
        expect {
          described_class.call(company_teammate: teammate, reward: reward)
        }.not_to change(HighlightsRedemption, :count)
      end
    end
    ...
    context 'with multiple redemptions' do
      let!(:reward2) { create(:highlights_reward, organization: organization, cost_in_points: 50) }
