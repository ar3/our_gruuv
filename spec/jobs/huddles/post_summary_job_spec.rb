...
RSpec.describe Huddles::PostSummaryJob, type: :job do
  ...
  describe '#perform' do
    context 'when Slack is configured' do
      ...
      it 'creates a new summary notification when none exists' do
        ...
        expect {
          described_class.perform_and_get_result(huddle.id)
        }.to change { huddle.notifications.summaries.count }.by(1)

        notification = huddle.notifications.summaries.last
      ...
      it 'updates existing summary when one exists' do
        # Create announcement first
        create(:notification, 
          notifiable: huddle,
          notification_type: 'huddle_announcement',
        ...
        new_notification = huddle.notifications.summaries.last
  ...
  describe 'perform_now execution' do
    ...
    it 'logs execution information' do
      ...
      expect(Rails.logger).to receive(:info).with(/Creating new summary for huddle #{huddle.id}/)
