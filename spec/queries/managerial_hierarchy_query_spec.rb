...
RSpec.describe ManagerialHierarchyQuery, type: :query do
  ...
  describe '#call' do
    ...
    context 'when person has a direct manager' do
      before do
        create(:employment_tenure, teammate: person_teammate, company: company, manager: direct_manager)
      end
    ...
    context 'when person has multiple managers' do
      ...
      let(:manager2_teammate) { create(:teammate, person: manager2, organization: company) }
      ...
      before do
        ...
        create(:employment_tenure, teammate: person_teammate, company: company, manager: manager2, started_at: 3.months.ago, ended_at: 2.months.ago)
        # Active tenure starting after the inactive one ended
        create(:employment_tenure, teammate: person_teammate, company: company, manager: direct_manager, started_at: 1.month.ago)
      end
      ...
      it 'only includes managers from active tenures' do
        ...
        other_teammate = create(:teammate, person: person, organization: other_company)
        inactive_manager = create(:person)
        create(:employment_tenure, teammate: other_teammate, company: other_company, manager: inactive_manager, started_at: 3.months.ago, ended_at: 2.months.ago)
    ...
    context 'when person has employment in different organization' do
      ...
      let(:other_company_teammate) { create(:teammate, person: person, organization: other_company) }

      before do
        create(:employment_tenure, teammate: person_teammate, company: company, manager: direct_manager)
        create(:employment_tenure, teammate: other_company_teammate, company: other_company, manager: other_company_manager)
      end
