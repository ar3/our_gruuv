...
RSpec.describe GoalsHelper, type: :helper do
  ...
  describe '#goal_category_label' do
    it 'returns "Vision" for vision goals' do
      goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'inspirational_objective', most_likely_target_date: nil)
      expect(helper.goal_category_label(goal)).to eq('Vision')
    end
    
    it 'returns "Objective" for objective goals' do
      goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'inspirational_objective', most_likely_target_date: Date.today + 90.days)
      expect(helper.goal_category_label(goal)).to eq('Objective')
    end
    
    it 'returns "Key Result" for key result goals' do
      goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'quantitative_key_result', most_likely_target_date: Date.today + 90.days)
      expect(helper.goal_category_label(goal)).to eq('Key Result')
    end
    
    it 'returns "Bad Key Result" for bad key result goals' do
      goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'quantitative_key_result', most_likely_target_date: nil)
  ...
  describe '#goal_warning_class' do
    it 'returns "table-danger" for goals that should show warning' do
      vision_goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'inspirational_objective', most_likely_target_date: nil)
      expect(helper.goal_warning_class(vision_goal)).to eq('table-danger')
      
      objective_goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'inspirational_objective', most_likely_target_date: Date.today + 90.days)
      expect(helper.goal_warning_class(objective_goal)).to eq('table-danger')
      
      bad_key_result_goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'quantitative_key_result', most_likely_target_date: nil)
    ...
    it 'returns empty string for goals that should not show warning' do
      goal2 = create(:goal, creator: creator_teammate, owner: person)
      vision_goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'inspirational_objective', most_likely_target_date: nil)
      ...
      key_result_goal = create(:goal, creator: creator_teammate, owner: person, goal_type: 'quantitative_key_result', most_likely_target_date: Date.today + 90.days)
