...
RSpec.describe InsightsHelper, type: :helper do
  ...
  describe '#og_scorecard_weekly_cell_class' do
    it 'maps statuses to Bootstrap table classes' do
      ...
      expect(helper.og_scorecard_weekly_cell_class(:warning)).to eq('table-warning')
      expect(helper.og_scorecard_weekly_cell_class(:danger)).to eq('table-danger')
      expect(helper.og_scorecard_weekly_cell_class(:neutral)).to eq('og-scorecard-cell-neutral')
    end
  end
end
