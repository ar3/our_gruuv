  ...
  describe '#navigation_structure' do
    ...
    describe 'Insights section' do
      ...
      it 'has the expected Insights sub-items with OG Scorecard first and Check-ins Progress last' do
        ...
        expect(labels.index('Milestones Health')).to eq(4)
        expect(labels.index('Observations Health')).to eq(5)
        expect(labels.index('Protect Flow')).to eq(6)
        expect(labels.index('Observations')).to eq(7)
    ...
    describe 'Admin/Explore MAAP(s), Admin (org essentials + admin), and Beta sections' do
      ...
      it 'includes Beta section with Insights, Position Comparison, Eligibility Requirements, and Goal Impact Scanner' do
        structure = helper.navigation_structure
