...
RSpec.describe NavigationHelper, type: :helper do
  ...
  describe '#navigation_structure' do
    ...
    describe 'About Me section' do
      ...
      it 'has the expected About Me sub-items including graduated pages from Beta' do
        structure = helper.navigation_structure
        about_me_section = structure.find { |item| item[:label] == 'About Me' }
        ...
        expect(labels[7]).to eq('Something Interesting')
        expect(labels[8]).to eq('Notifications')
        expect(labels[9]).to eq('My Growth')
        expect(labels[10]).to eq('My Huddles')
      end
    end
    ...
    describe 'Insights section' do
      ...
      it 'has the expected Insights sub-items with OG Scorecard first and Check-ins Progress last' do
        structure = helper.navigation_structure
        insights_section = structure.find { |item| item[:label] == 'Insights' }
        ...
        expect(labels.index('Real OG Leaders')).to eq(1)
        expect(labels.index('Who is doing what')).to eq(2)
        expect(labels.index('Check-ins Health')).to eq(3)
    ...
    describe 'Admin/Explore MAAP(s), Admin (org essentials + admin), and Beta sections' do
      ...
      it 'includes Beta section with Insights, Position Comparison, Eligibility Requirements, Position Suggestions, and Coach Inbox' do
        ...
        expect(labels).not_to include('My Growth')
      ...
      it 'includes graduated About Me items outside Beta' do
        structure = helper.navigation_structure
        about_me = structure.find { |item| item[:section] == 'about_me' }
        labels = about_me[:items].map { |item| item[:label] }
        expect(labels).to include('Something Interesting')
        expect(labels).to include('Notifications')
        expect(labels).to include('My Growth')
      end

      it 'includes Bulk award milestones under Celebrate Milestones' do
        structure = helper.navigation_structure
        section = structure.find { |item| item[:section] == 'celebrate_milestones' }
        expect(section).to be_present
        labels = section[:items].map { |item| item[:label] }
        expect(labels).to include('Celebrate Milestones')
    ...
    describe 'Goals section' do
      it 'includes Goals section with Add New, personal, team, department goals, and Hierarchy Map' do
        structure = helper.navigation_structure
        ...
        expect(labels).to eq(['Add New Goals', 'My personal goals', 'My team goals', 'My department goals', 'Goals Hierarchy Map'])
        ...
        expect(goals_section[:items][4][:path]).to eq(helper.organization_goal_impact_scanner_path(company))
      end
    end
  end
