...
RSpec.describe AboutMeHelper, type: :helper do
  ...
  describe '#goals_status_indicator' do
    def create_goal_confidence_status(status)
    ...
    it 'returns :red when EngagementHealth Goal Confidence is missing' do
      expect(helper.goals_status_indicator(company_teammate)).to eq(:red)
    end

    it 'returns :green for Healthy' do
      create_goal_confidence_status(EngagementHealth::HEALTHY)
      expect(helper.goals_status_indicator(company_teammate)).to eq(:green)
    end

    it 'returns :yellow for Warning' do
      create_goal_confidence_status(EngagementHealth::WARNING)
      expect(helper.goals_status_indicator(company_teammate)).to eq(:yellow)
    end

    it 'returns :red for Needs Attention' do
      create_goal_confidence_status(EngagementHealth::NEEDS_ATTENTION)
      expect(helper.goals_status_indicator(company_teammate)).to eq(:red)
    end
  end
