...
RSpec.describe CheckInHelper, type: :helper do
  ...
  describe '#assignment_energy_sentence' do
    let(:organization) { create(:organization) }
    let(:person) { create(:person, first_name: 'Sam', last_name: 'Test') }
    ...
    it 'returns sentence with bold casual name, assignment title, and energy' do
      sentence = helper.assignment_energy_sentence(check_in)
    ...
    it 'returns empty string when check_in is nil' do
      expect(helper.assignment_energy_sentence(nil)).to eq('')
    end

    it 'returns empty string when energy is blank' do
      check_in.actual_energy_percentage = nil
      expect(helper.assignment_energy_sentence(check_in)).to eq('')
    end
  end
