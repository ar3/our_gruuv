...
RSpec.describe 'Profile Management', type: :system, js: true do
  ...
  before do
    # Use proper authentication for system specs (works with JS tests)
  ...
  it 'User can update their profile information' do
    ...
    # Wait for redirect
    expect(page).to have_current_path(profile_path, wait: 5)
    
    # Success message may be in a toast/alert - check for updated content instead
    expect(page).to have_content('Jane Marie Smith Jr.', wait: 5)
