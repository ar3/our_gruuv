...
RSpec.describe PositionForm, type: :form do
  ...
  describe 'validations' do
    it 'requires title_id' do
      params = { title_id: nil, position_level_id: position_level.id, version_type: 'ready' }
      form.instance_variable_set(:@form_data_empty, false)
      expect(form.validate(params)).to be false
      expect(form.errors[:title_id]).to include("can't be blank")
    end

    it 'requires position_level_id' do
      params = { title_id: title.id, position_level_id: nil, version_type: 'ready' }
      form.instance_variable_set(:@form_data_empty, false)
      expect(form.validate(params)).to be false
    ...
    it 'requires version_type for new records' do
      params = { title_id: title.id, position_level_id: position_level.id, version_type: nil }
      form.instance_variable_set(:@form_data_empty, false)
      expect(form.validate(params)).to be false
  ...
  describe 'version calculation' do
    context 'for new records' do
      it 'sets version to 1.0.0 for ready' do
        params = { title_id: title.id, position_level_id: position_level.id, version_type: 'ready' }
        form.instance_variable_set(:@form_data_empty, false)
      ...
      it 'sets version to 0.0.1 for early_draft' do
        params = { title_id: title.id, position_level_id: position_level.id, version_type: 'early_draft' }
        form.instance_variable_set(:@form_data_empty, false)
        expect(form.validate(params)).to be true
        expect(form.save).to be true
    ...
    context 'for existing records' do
      let(:existing_position) { create(:position, title: title, position_level: position_level, semantic_version: '2.1.3') }
      ...
      it 'bumps major version for fundamental change' do
        params = { title_id: title.id, position_level_id: position_level.id, version_type: 'fundamental' }
        expect(form.validate(params)).to be true
        expect(form.save).to be true
      ...
      it 'bumps minor version for clarifying change' do
        params = { title_id: title.id, position_level_id: position_level.id, version_type: 'clarifying' }
        expect(form.validate(params)).to be true
        expect(form.save).to be true
      ...
      it 'bumps patch version for insignificant change' do
        params = { title_id: title.id, position_level_id: position_level.id, version_type: 'insignificant' }
        expect(form.validate(params)).to be true
        expect(form.save).to be true
