...
RSpec.describe 'Company teammate bulk milestone awards', type: :request do
  ...
  describe 'when manager awards a report' do
    ...
    it 'GET new renders the form and milestone radios' do
      ...
      expect(response.body).to include('bulkMilestoneAwardPageInfo')
      expect(response.body).to include('Show page explanation')
      expect(response.body).to include('Ultimate goal')
      expect(response.body).to include('Use it once per person')
      expect(response.body).to include('anti-pattern in OG')
    ...
    it 'POST review renders the review step (wizard page 2)' do
      ...
      expect(response.body).to include('bulkMilestoneAwardReviewPageInfo')
