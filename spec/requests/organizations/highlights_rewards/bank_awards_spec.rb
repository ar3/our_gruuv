...
RSpec.describe 'Organizations::HighlightsRewards::BankAwards', type: :request do
  let(:organization) { create(:organization) }
  let(:banker_person) { create(:person) }
  let(:banker_teammate) do
    create(:company_teammate, person: banker_person, organization: organization, can_manage_highlights_rewards: true)
  end
  ...
  describe 'GET /organizations/:organization_id/highlights_rewards/bank_awards/new' do
    context 'when user has can_manage_highlights_rewards' do
      before { sign_in_as_teammate_for_request(banker_person, organization) }

      it 'returns success' do
        get new_organization_highlights_rewards_bank_award_path(organization)
    ...
    context 'when user does not have can_manage_highlights_rewards' do
      before do
        banker_teammate.update!(can_manage_highlights_rewards: false)
  ...
  describe 'POST /organizations/:organization_id/highlights_rewards/bank_awards' do
    context 'when user has can_manage_highlights_rewards' do
      before { sign_in_as_teammate_for_request(banker_person, organization) }

      it 'creates a bank award and redirects to index with success' do
        expect {
          post organization_highlights_rewards_bank_awards_path(organization),
               params: {
                 bank_award: {
        ...
        expect(response).to have_http_status(:redirect)
        expect(response).to redirect_to(organization_highlights_rewards_bank_awards_path(organization))
      ...
      it 'applies points to the recipient ledger' do
        ...
        expect(response).to redirect_to(organization_highlights_rewards_bank_awards_path(organization))
        ledger = recipient_teammate.highlights_ledger.reload
      ...
      context 'with invalid recipient' do
        it 'redirects back to new with alert' do
          ...
          expect(response).to have_http_status(:redirect)
          expect(response).to redirect_to(new_organization_highlights_rewards_bank_award_path(organization))
      ...
      context 'when service returns an error' do
        before do
          allow(Highlights::AwardBankPointsService).to receive(:call).and_return(
