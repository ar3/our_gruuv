...
RSpec.describe 'About Me Page', type: :request do
  ...
  describe 'Onboarding Spotlight removed' do
    it 'does not show the legacy Welcome to OurGruuv onboarding spotlight' do
      get about_me_organization_company_teammate_path(organization, teammate)
      expect(response).to have_http_status(:success)
      expect(response.body).not_to include('Welcome to OurGruuv!')
      expect(response.body).not_to include('Adding a Kudos, Feedback, or Observation')
    end
  end
