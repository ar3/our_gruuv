...
RSpec.describe 'Organizations::Insights', type: :request do
  ...
  describe 'GET /organizations/:organization_id/insights/who_is_doing_what' do
    ...
    it 'renders Who is doing what insights page with pie, top pages, histogram, and period stats' do
      ...
      expect(response.body).to include('Digest configuration coverage')
      expect(response.body).to include('who-is-doing-what-weekly-digest-day-chart')
      expect(response.body).to include('who-is-doing-what-weekly-digest-type-chart')
      expect(response.body).to include('who-is-doing-what-daily-digest-type-chart')
      expect(response.body).to include('who-is-doing-what-digest-medium-chart')
      expect(response.body).to include('Weekly digest day of week')
      expect(response.body).to include('1:1 guide vs About Me')
      expect(response.body).to include('Interesting Things vs GSD')
      expect(response.body).to include('Digest mediums (Slack / SMS / Email)')
  ...
  describe 'GET /organizations/:organization_id/insights/og_scorecard' do
    ...
    it 'renders scorecard table, timeframe controls, and column headers' do
      ...
      expect(response.body).to include('published an OGO this week')
      expect(response.body).to include('published an OGO all-time')
      expect(response.body).to include('observees in an OGO this week')
      expect(response.body).to include('observees in an OGO all-time')
      expect(response.body).not_to include('within 30 days of this week')
