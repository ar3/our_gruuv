...
RSpec.describe 'Observation Creation Wizard', type: :request do
  ...
  before do
    ...
    # Disable PaperTrail for request tests
    allow(PaperTrail).to receive(:enabled?).and_return(false)
  end

  # Helper method to set session data for request specs
  def set_session_data(data)
    # Use a more specific approach that doesn't interfere with PaperTrail
    allow_any_instance_of(ActionDispatch::Request).to receive(:session).and_return(data)
    allow_any_instance_of(ActionDispatch::Request).to receive(:session).and_return(data)
  end
  ...
  describe 'Step 1: Who, When, What, How' do
    ...
    context 'with invalid step 1 data' do
      ...
      it 'renders step 1 with validation errors' do
        post organization_observations_path(company), params: invalid_step1_params
  ...
  describe 'Step 2: Ratings & Privacy' do
    ...
    before do
      set_session_data({ 'observation_wizard_data' => wizard_data })
    end
  ...
  describe 'Step 3: Review & Manage' do
    ...
    context 'POST create_observation' do
      ...
      it 'creates observation and redirects to show page' do
        expect {
          post create_observation_organization_observation_path(company, 'new'), params: final_params
