...
RSpec.describe 'Organizations::Employees#customize_view', type: :request do
  ...
  describe 'check-in status display options' do
    context 'when user has manage_employment permission' do
      ...
      it 'does not show Check-ins Health Style 2 option' do
        get customize_view_organization_employees_path(organization)

        expect(response).to be_successful
        expect(response.body).not_to include('Check-ins Health Style 2')
        expect(response.body).not_to include('id="view_check_ins_health"')
      end
    end
  ...
  describe 'check-ins health spotlight options' do
    it 'does not show legacy Check-ins Health Style spotlight options' do
      get customize_view_organization_employees_path(organization)

      expect(response).to be_successful
      expect(response.body).not_to include('Check-ins Health Style 1')
      expect(response.body).not_to include('Check-ins Health Style 2')
      expect(response.body).not_to include('id="spotlight_check_ins_health_style_1"')
      expect(response.body).not_to include('id="spotlight_check_ins_health_style_2"')
    end
  end
  ...
  describe 'presets' do
    ...
    it 'still includes My Direct Reports Style 1 preset and not Style 2' do
      get customize_view_organization_employees_path(organization)
      
      expect(response).to be_successful
      expect(response.body).to include('My Direct Reports - Check-in Status Style 1')
      expect(response.body).not_to include('My Direct Reports - Check-in Status Style 2')
    end
  end
