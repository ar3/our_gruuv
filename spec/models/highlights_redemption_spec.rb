...
RSpec.describe HighlightsRedemption, type: :model do
  ...
  describe 'validations' do
    it 'is valid with proper attributes' do
      redemption = build(:highlights_redemption,
    ...
    it 'requires points_spent' do
      redemption = build(:highlights_redemption, points_spent: nil)
      expect(redemption).not_to be_valid
    end

    it 'requires points_spent to be positive' do
      redemption = build(:highlights_redemption, points_spent: 0)
      expect(redemption).not_to be_valid
    end

    it 'requires points_spent to be in 0.5 increments' do
      redemption = build(:highlights_redemption, points_spent: 10.3)
      expect(redemption).not_to be_valid
      expect(redemption.errors[:points_spent]).to include("must be in 0.5 increments")
    end

    it 'requires a valid status' do
      redemption = build(:highlights_redemption, status: 'invalid')
      expect(redemption).not_to be_valid
    end

    it 'requires reward to be in the same organization' do
      other_org = create(:organization)
      other_reward = create(:highlights_reward, organization: other_org)
      # Build directly without factory to avoid auto-correction
      redemption = HighlightsRedemption.new(
        organization: organization,
        company_teammate: teammate,
        highlights_reward: other_reward,
        points_spent: 100,
        status: 'pending')
      expect(redemption).not_to be_valid
      expect(redemption.errors[:highlights_reward]).to include("must belong to the same organization")
    end
  ...
  describe 'scopes' do
    let!(:pending_redemption) { create(:highlights_redemption, :pending, organization: organization, company_teammate: teammate, highlights_reward: reward) }
    let!(:fulfilled_redemption) { create(:highlights_redemption, :fulfilled, organization: organization, company_teammate: teammate, highlights_reward: reward) }
    let!(:cancelled_redemption) { create(:highlights_redemption, :cancelled, organization: organization, company_teammate: teammate, highlights_reward: reward) }

    describe '.pending' do
      it 'returns only pending redemptions' do
        expect(HighlightsRedemption.pending).to include(pending_redemption)
        expect(HighlightsRedemption.pending).not_to include(fulfilled_redemption)
      end
    end

    describe '.fulfilled' do
      it 'returns only fulfilled redemptions' do
        expect(HighlightsRedemption.fulfilled).to include(fulfilled_redemption)
        expect(HighlightsRedemption.fulfilled).not_to include(pending_redemption)
      end
    end

    describe '.active' do
      it 'returns pending and processing redemptions' do
        expect(HighlightsRedemption.active).to include(pending_redemption)
        expect(HighlightsRedemption.active).not_to include(fulfilled_redemption, cancelled_redemption)
      end
    end

    describe '.completed' do
      it 'returns fulfilled, failed, and cancelled redemptions' do
        expect(HighlightsRedemption.completed).to include(fulfilled_redemption, cancelled_redemption)
        expect(HighlightsRedemption.completed).not_to include(pending_redemption)
      end
    end
  end

  describe 'state transitions' do
    let(:redemption) { create(:highlights_redemption, :pending, organization: organization, company_teammate: teammate, highlights_reward: reward) }
    ...
    describe '#mark_processing!' do
      ...
      it 'raises error if not pending' do
        ...
        expect { redemption.mark_processing! }.to raise_error(HighlightsRedemption::InvalidStateTransition)
      end
    end
    ...
    describe '#mark_cancelled!' do
      ...
      it 'raises error if already fulfilled' do
        ...
        expect { redemption.mark_cancelled! }.to raise_error(HighlightsRedemption::InvalidStateTransition)
      end
    end
  end
