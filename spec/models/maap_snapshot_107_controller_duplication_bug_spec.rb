...
RSpec.describe MaapSnapshot, type: :model do
  ...
  before do
    ...
    # Set up check-ins with existing data matching snapshot 107
  ...
  describe 'build_official_check_in_data_with_changes' do
    context 'when form params include check_in_data hash like in bulk_finalize_check_ins controller' do
      it 'should fail by producing the buggy behavior from snapshot 107' do
        # Simulate the exact form params that the controller passes to MaapSnapshot
        # This includes both the original form params AND the check_in_data hash
        ...
        check_in_data = {
          check_in1.id => {
            ...
            shared_notes: 'life - work',
            close_rating: false
          },
          check_in2.id => {
            check_in_id: check_in2.id,
            final_rating: 'meeting',
            shared_notes: 'emp gro - meet',
            close_rating: false
          }
        ...
        snapshot = MaapSnapshot.build_for_employee_with_changes(
          ...
          reason: 'Testing snapshot 107 controller duplication bug',
