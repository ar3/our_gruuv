...
RSpec.describe 'BulkCheckInFinalizationProcessor Snapshot 112 Exact Duplication Bug', type: :model do
  ...
  describe 'Snapshot 112 exact duplication bug in processor' do
    context 'when check_in_data has check_in_id matching assignment_id' do
      it 'should fail by reproducing the exact duplication bug from snapshot 112' do
        # Form params that exactly match snapshot 112:
        # The check_in_id in check_in_data is actually the assignment_id, not the check-in ID
        form_params = {
          "check_in_data" => {
            "80" => {
        ...
        snapshot = MaapSnapshot.build_for_employee_without_maap_data(
          ...
          reason: 'Testing snapshot 112 exact duplication bug in processor',
        ...
        puts "\n=== PROCESSOR SNAPSHOT 112 EXACT DUPLICATION BUG ==="
