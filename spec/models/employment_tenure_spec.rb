...
RSpec.describe EmploymentTenure, type: :model do
  ...
  describe 'overlapping tenures validation' do
    ...
    it 'prevents overlapping active tenures for same person and company' do
      ...
      expect(overlapping_tenure.errors[:base]).to include('Cannot have overlapping active employment tenures for the same teammate and company')
    end
  ...
  describe '.most_recent_for' do
    ...
    it 'returns nil when no employment tenures exist' do
      other_person = create(:person)
      other_teammate = create(:teammate, person: other_person, organization: company)
      result = EmploymentTenure.most_recent_for(other_teammate, company)
