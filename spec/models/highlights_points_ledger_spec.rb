...
RSpec.describe HighlightsPointsLedger, type: :model do
  ...
  describe 'validations' do
    ...
    subject { build(:highlights_points_ledger, company_teammate: company_teammate) }
    ...
    context 'half increment validation' do
      it 'allows whole numbers' do
        ledger = build(:highlights_points_ledger, company_teammate: company_teammate, points_to_give: 10.0)
        expect(ledger).to be_valid
      end

      it 'allows half increments' do
        ledger = build(:highlights_points_ledger, company_teammate: company_teammate, points_to_give: 10.5)
        expect(ledger).to be_valid
      end

      it 'rejects quarter increments' do
        ledger = build(:highlights_points_ledger, company_teammate: company_teammate, points_to_give: 10.25)
  ...
  describe 'scopes' do
    ...
    before do
      create(:highlights_points_ledger, company_teammate: teammate1, organization: organization, points_to_give: 50)
      create(:highlights_points_ledger, company_teammate: teammate2, organization: organization, points_to_give: 0, points_to_spend: 0)
    end

    describe '.for_organization' do
      it 'returns ledgers for the specified organization' do
        expect(HighlightsPointsLedger.for_organization(organization).count).to eq(2)
      end
    end

    describe '.with_balance' do
      it 'returns only ledgers with points' do
        expect(HighlightsPointsLedger.with_balance.count).to eq(1)
      end
    end
  end

  describe 'instance methods' do
    let(:ledger) { create(:highlights_points_ledger, points_to_give: 50.0, points_to_spend: 25.0) }
    ...
    describe '#deduct_from_give' do
      ...
      it 'raises InsufficientBalance when not enough points' do
        expect { ledger.deduct_from_give(100.0) }.to raise_error(HighlightsPointsLedger::InsufficientBalance)
      end
    end
    ...
    describe '#deduct_from_spend' do
      ...
      it 'raises InsufficientBalance when not enough points' do
        expect { ledger.deduct_from_spend(100.0) }.to raise_error(HighlightsPointsLedger::InsufficientBalance)
      end
    end
  ...
  describe '.find_or_create_for' do
    ...
    it 'creates a new ledger if none exists' do
      expect {
        HighlightsPointsLedger.find_or_create_for(company_teammate, organization)
      }.to change(HighlightsPointsLedger, :count).by(1)
    end

    it 'returns existing ledger if one exists' do
      existing = create(:highlights_points_ledger, company_teammate: company_teammate, organization: organization)
      found = HighlightsPointsLedger.find_or_create_for(company_teammate, organization)
