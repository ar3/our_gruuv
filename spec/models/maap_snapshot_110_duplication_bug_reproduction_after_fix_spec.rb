...
RSpec.describe 'MaapSnapshot Snapshot 110 Duplication Bug Reproduction', type: :model do
  ...
  describe 'Snapshot 110 duplication bug reproduction' do
    context 'when using the exact form_params from snapshot 110' do
      it 'should now work correctly after fixing the check_in_data format processing' do
        ...
        # Create a snapshot using the OLD method (this should now work correctly)
        snapshot = MaapSnapshot.build_for_employee_with_changes(
          employee: employee,
          created_by: employee,
          change_type: 'bulk_check_in_finalization',
          reason: 'Testing snapshot 110 duplication bug reproduction AFTER FIX',
        ...
        puts "\n=== SNAPSHOT 110 DUPLICATION BUG REPRODUCTION AFTER FIX ==="
        ...
        # Expected behavior AFTER FIX: The check_in_data format should be processed correctly
        ...
        # This spec should PASS because the check_in_data format is now being processed correctly
        # The bug has been fixed: no more cross-assignment duplication
      end
    end
  end
end
