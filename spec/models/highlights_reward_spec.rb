...
RSpec.describe HighlightsReward, type: :model do
  let(:organization) { create(:organization) }

  describe 'validations' do
    it 'is valid with proper attributes' do
      reward = build(:highlights_reward, organization: organization)
      expect(reward).to be_valid
    end

    it 'requires a name' do
      reward = build(:highlights_reward, name: nil)
      expect(reward).not_to be_valid
      expect(reward.errors[:name]).to include("can't be blank")
    end

    it 'requires a cost_in_points' do
      reward = build(:highlights_reward, cost_in_points: nil)
      expect(reward).not_to be_valid
    end

    it 'requires cost_in_points to be positive' do
      reward = build(:highlights_reward, cost_in_points: 0)
      expect(reward).not_to be_valid
    ...
    it 'requires cost_in_points to be in 0.5 increments' do
      reward = build(:highlights_reward, cost_in_points: 10.3)
      expect(reward).not_to be_valid
      expect(reward.errors[:cost_in_points]).to include("must be in 0.5 increments")
    end

    it 'allows cost_in_points in 0.5 increments' do
      reward = build(:highlights_reward, cost_in_points: 10.5)
      expect(reward).to be_valid
    end

    it 'requires a valid reward_type' do
      reward = build(:highlights_reward, reward_type: 'invalid')
      expect(reward).not_to be_valid
    end
  end

  describe 'scopes' do
    let!(:active_reward) { create(:highlights_reward, organization: organization, active: true) }
    let!(:inactive_reward) { create(:highlights_reward, :inactive, organization: organization) }
    let!(:deleted_reward) { create(:highlights_reward, :deleted, organization: organization) }
    let!(:cheap_reward) { create(:highlights_reward, :cheap, organization: organization) }
    let!(:expensive_reward) { create(:highlights_reward, :expensive, organization: organization) }

    describe '.active' do
      it 'returns only active, non-deleted rewards' do
        expect(HighlightsReward.active).to include(active_reward, cheap_reward, expensive_reward)
        expect(HighlightsReward.active).not_to include(inactive_reward, deleted_reward)
      end
    end
    ...
    describe '.for_organization' do
      it 'filters by organization' do
        ...
        expect(HighlightsReward.for_organization(organization)).to include(active_reward)
        expect(HighlightsReward.for_organization(organization)).not_to include(other_reward)
      end
    end

    describe '.affordable_with' do
      it 'returns rewards that cost less than or equal to the given points' do
        affordable = HighlightsReward.affordable_with(100)
  ...
  describe 'instance methods' do
    ...
    describe 'type helpers' do
      it '#gift_card? returns true for gift_card type' do
        reward = build(:highlights_reward, :gift_card)
      ...
      it '#merchandise? returns true for merchandise type' do
        reward = build(:highlights_reward, :merchandise)
        expect(reward.merchandise?).to be true
      end
    end

    describe 'metadata helpers' do
      let(:reward) { create(:highlights_reward, :gift_card, organization: organization) }
