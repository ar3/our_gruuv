...
RSpec.describe GoalPolicy, type: :policy do
  ...
  let(:personal_goal) { create(:goal, creator: creator_teammate, owner: creator_teammate, privacy_level: 'only_creator') }
  ...
  describe 'update?' do
    ...
    context 'when user is owner (Teammate)' do
      it 'allows owner to update their own goal' do
        goal = create(:goal, creator: creator_teammate, owner: owner_teammate, privacy_level: 'only_creator_and_owner')
        policy = GoalPolicy.new(pundit_user_owner, goal)
        expect(policy.update?).to be true
      end
    end
    ...
    context 'when owner is Organization' do
      ...
      it 'denies non-member to update' do
        # other_teammate is in the same company, so they can view but should not be able to update
        # unless they are the creator or directly in the owner organization
  ...
  describe 'scope' do
    let!(:personal_goal1) { create(:goal, creator: creator_teammate, owner: creator_teammate, privacy_level: 'only_creator_and_owner') }
    let!(:personal_goal2) { create(:goal, creator: owner_teammate, owner: owner_teammate, privacy_level: 'only_creator_and_owner') }
    let!(:personal_goal_private) { create(:goal, creator: creator_teammate, owner: owner_teammate, privacy_level: 'only_creator') }
    let!(:personal_goal_managers) { create(:goal, creator: creator_teammate, owner: owner_teammate, privacy_level: 'only_creator_owner_and_managers') }
    let!(:personal_goal_everyone) { create(:goal, creator: creator_teammate, owner: owner_teammate, privacy_level: 'everyone_in_company') }
    let!(:org_goal) { create(:goal, creator: creator_teammate, owner: company, privacy_level: 'everyone_in_company') }
    ...
    context 'when user is a teammate' do
      it 'returns all goals in the same company (simplified scope - UI handles filtering)' do
        policy = GoalPolicy::Scope.new(pundit_user_creator, Goal)
        resolved = policy.resolve
        
        # Should include all goals in the same company
        expect(resolved).to include(personal_goal1, personal_goal2, personal_goal_private, personal_goal_managers, personal_goal_everyone, org_goal, org_goal_private)
        # Should not include goals from other companies
        expect(resolved).not_to include(other_company_goal)
      end
    end
