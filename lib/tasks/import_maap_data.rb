...
class MaapDataImporter
  ...
  def create_position_data
    ...
    @position_levels = {}
    [1, 2, 3].each do |level|
      position_level = PositionLevel.find_or_create_by(level: "2.#{level}", position_major_level: @position_major_level) do |pl|
      ...
      if position_level.persisted? && position_level.previously_new_record?
        puts "  ✅ Created position level: Level 2.#{level}"
        @created_count += 1
      else
        puts "  ⏭️  Position level already exists: Level 2.#{level}"
        @skipped_count += 1
      end
    end
